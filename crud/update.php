<?php
	...
	if (empty($birthdate)) {
            $birthdateError = 'Please enter Bithdate';
            $valid = false;
        }
	...
	if (empty($user_name)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE customer set name = ?, last_name = ?, birthdate = ?, phone_number = ?, email_address = ?, user_name = ?, password = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($name,$last_name,$birthdate,$phone_number,$smail_address,$user_name,$password,$id));
            Database::disconnect();
            header("Location: index.php");
        }
    } else {
